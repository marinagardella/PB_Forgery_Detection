...
template <typename T>
void sortArr(T arr[], int brr[], int n) 
{ 
  
    // Vector to store element 
    // with respective present index 
    std::vector<pair<T, int>> vp; 
  
    // Inserting element in pair vector 
    // to keep track of previous indexes
    std::vector<int> brr_vec; 
    for (int i = 0; i < n; ++i) { 
        vp.push_back(make_pair(arr[i], i)); 
        brr_vec.push_back(brr[i]);
    } 
  
    // Sorting pair vectorcompute_VH
    std::sort(vp.begin(), vp.end());

    for (int i = 0; i < n; ++i) {
        arr[i] = vp[i].first;
        brr[i] = brr_vec[vp[i].second];
    }
}
