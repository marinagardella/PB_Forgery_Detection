 ...
 *
 * Copyright(c) 2011 Miguel Colom and Marina Gardella
...
void algorithm(int argc, char **argv) {
  ...
  OptStruct oMblockSize   =  {"a:", 512, "512", NULL, "Macroblock side"};
  options.push_back(&oMblockSize);

  OptStruct oStride   =  {"o:", 0, "0.5", NULL, "Macroblock stride"};
  options.push_back(&oStride);
  ...
  int mblock_size = atoi(oMblockSize.value);
  float mblock_stride = atof(oStride.value);

  //printf("mblock_size=%d, mblock_stride=%f, w=%d\n", mblock_size, mblock_stride, w);
  ...
  // Load input image
  CImage input_big;
  input_big.load((char*)pinput.value);

  // Get image properties
  int Nx_big = input_big.get_width();
  int Ny_big = input_big.get_height();
  int num_channels = input_big.get_num_channels();
  ...
  if (num_bins <= 0) {
    fprintf(stderr, "Error: please specify number of bins\n");
    exit(-1);
  }
  ...
  fftwf_r2r_kind kindTable[2] = {FFTW_REDFT10, FFTW_REDFT10};
    ...
    for (int y = 0; y < Ny_big- mblock_size + 1 ; y += int(mblock_size * mblock_stride)) { //reste mblock_size
      for (int x = 0; x < Nx_big - mblock_size + 1 ; x += int(mblock_size * mblock_stride)) { //reste mblock_size
	    int Nx = mblock_size;
	    int Ny = mblock_size;

	    // Adjust size of the macroblock when it's close to the borders
	    int diff = x + mblock_size - Nx_big;
	    if (diff > 0) Nx -= diff;
	    //
	    diff = y + mblock_size - Ny_big;
	    if (diff > 0) Ny -= diff;

            CImage input(Nx, Ny, input_big.get_bits_per_channel(), num_channels);

	    // Extraemos el macrobloque dentro de input (los 3 canales)
	    for (int ch = 0; ch < num_channels; ch++) {
              float *u = input.get_channel(ch);
	      float *v = input_big.get_channel(ch);
     	      for (int j = 0; j < Ny; j++) {
	        for (int i = 0; i < Nx; i++) {
		  u[i+Nx*j] = v[Nx_big*(y+j)+(x+i)];
	        }
	      }
	    }
	  ...
	  // Process each channel
		...
		if (elems_bin == 0) continue; // skip empty bins
		...
		// Compute VL
		  ...
		  K = get_optimal_K_ponom_orig(elems_bin, VL);
	        ...
	        // Store results
		//printf("ch=%d, bin=%d, bin_mean=%f, tilde_sigma=%f, idx=%d\n", ch, bin, bin_mean, tilde_sigma, ch*num_bins+bin);
	  ...
	  //
	  for (int ch = 0; ch < num_channels; ch++)
	    for (int filt_iter = 0; filt_iter < curve_filter_iterations*0; filt_iter++) {
